...
class CheckAuthorizationToken
{
    ...
    public function handle($request, Closure $next)
    {
        ...
        if (empty($authToken)) {
            return response()->json(['error' => 'Empty token and Unauthorized'], 401);
        } else {
            $accessToken = $this->findUserAccessToken($authToken);
            try {
                if (!$accessToken) {
                    return response()->json(['error' => 'Token sent and Unauthorized'], 401);
                }
                return $next($request);
            } catch (Exception $e) {
                return response()->json(['error3' => 'Error checking Unauthorized'], 401);
            }
        }
        return $next($request);
    }
