                ...
                <div class="col-md-7 border-right p-4">
                    ...
                    @php
                        if (!empty($lastTransaction)) {
                            ...
                            } elseif ($lastTransaction->transaction_type->name == 'Payment_Sent') {
                                $transactionName = 'Payment Sent';
                            } elseif ($lastTransaction->transaction_type->name == 'Payment_Received') {
                                $transactionName = 'Payment Received';
                            } elseif ($lastTransaction->transaction_type->name == 'Crypto_Sent') {
                                $transactionName = 'Crypto Sent';
                            } elseif ($lastTransaction->transaction_type->name == 'Crypto_Received') {
                                $transactionName = 'Crypto Received';
                            } else {
                    ...
                    @else 
                        @if (!empty($lastTransaction))
                        <h5>{{ __('Your last transaction was') }} <span class="text-primary">{{ number_format($lastTransaction->subtotal, '2', '.', '') }} {{ $lastTransaction->currency->code }} </span> <span style="font-size:small;">( {{ $transactionName }} )</span> </h5>
                        @else
                                                        ...
                                                        <tr  click="0" data-toggle="modal" data-target="#collapseRow{{$key}}" aria-expanded="false" aria-controls="collapseRow{{$key}}" class="show_area cursor-pointer" trans-id="{{$transaction->id}}" id="{{$key}}">
                                                            ...
                                                            @if($transaction->subtotal > 0)
                                                                <td class="text-right pr-5">
                                                                    <p><span class="text-16 font-weight-600"> +{{ formatNumber($transaction->subtotal) }}</span> <span class="c-code">({{ $transaction->currency->code }})</span></p>
                                                                </td>
                                                            @endif
                                                            @elseif($transaction->transaction_type_id == Withdrawal)
                                                                <td class="text-right pr-5">
                                                            ...
                                                            @else
                                                                @if($transaction->total > 0)
                                                                    <td class="text-right pr-5">
                                                                        <p> <span class="text-16 font-weight-600"> {{ $transaction->currency->type != 'fiat' ? "+".$transaction->total : "+".formatNumber($transaction->total) }} </span> <span class="c-code">({{ $transaction->currency->code }})</span></p>
                                                                    </td>
                                                                @elseif($transaction->total == 0)
                                                                    <td class="text-right pr-5">
                                                                        <p><span class="text-16 font-weight-600"> {{ formatNumber($transaction->total) }} </span><span class="c-code">({{ $transaction->currency->code }})</span></p>
                                                                    </td>
                                                                @elseif($transaction->total < 0)
                                                                    <td class="text-right pr-5">
                                                                        <p><span class="text-16 font-weight-600">{{ $transaction->currency->type != 'fiat' ? $transaction->total : formatNumber($transaction->total) }} </span> <span class="c-code">({{ $transaction->currency->code }})</span></p>
