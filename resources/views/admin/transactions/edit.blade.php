			...
			<div>
				@if ($transaction->status)
					...
					@elseif ($transaction->transaction_type_id == Crypto_Received)
							@if ($transaction->status == 'Success')<span class="text-green">Success</span>@endif
					@elseif (config('crypto_exchange.is_active') && $transaction->transaction_type_id == Crypto_Exchange_From)
							@if ($transaction->status == 'Success')<span class="text-green">Success</span>@endif
							@if ($transaction->status == 'Pending')<span class="text-blue">Pending</span>@endif
					@elseif (config('crypto_exchange.is_active') && $transaction->transaction_type_id == Crypto_Exchange_To)
							@if ($transaction->status == 'Success')<span class="text-green">Success</span>@endif
					@elseif (config('crypto_exchange.is_active') && $transaction->transaction_type_id == Fiat_Exchange_From)
							@if ($transaction->status == 'Success')<span class="text-green">Success</span>@endif
							@if ($transaction->status == 'Pending')<span class="text-blue">Pending</span>@endif
					@elseif (config('crypto_exchange.is_active') && $transaction->transaction_type_id == Fiat_Exchange_To)
							@if ($transaction->status == 'Success')<span class="text-green">Success</span>@endif
					@endif</h4>
				@endif
			</div>
                                            ...
                                            @endif
                                                ...
                                                {{-- User --}}
                                                    <div class="form-group">
                                                            ...
                                                            @elseif(config('crypto_exchange.is_active') && $transaction->transaction_type_id == Crypto_Exchange_From || $transaction->transaction_type_id == Crypto_Exchange_To || $transaction->transaction_type_id == Fiat_Exchange_From || $transaction->transaction_type_id == Fiat_Exchange_To)
                                                                <label class="control-label col-sm-3" for="user">Exchanger</label>
                                                        ...
                                                        <div class="col-sm-9">
                                                        @php
                                                            $transactionTypeName = [Deposit, Transferred, Exchange_From, Exchange_To, Request_From, Withdrawal, Payment_Sent, Crypto_Sent];

                                                            if (config('crypto_exchange.is_active')) {
                                                                array_push($transactionTypeName, Crypto_Exchange_From, Crypto_Exchange_To, Fiat_Exchange_From, Fiat_Exchange_To);
                                                            }
                                                        @endphp
                                                            <p class="form-control-static">
                                                            @if (in_array($transaction->transaction_type_id, $transactionTypeName))
                                                                {{ isset($transaction->user) ? $transaction->user->first_name.' '.$transaction->user->last_name :"-" }}
                                                            @elseif (in_array($transaction->transaction_type_id, [Received, Request_To, Payment_Received, Crypto_Received]))
                                                                {{ isset($transaction->end_user) ? $transaction->end_user->first_name.' '.$transaction->end_user->last_name :"-" }}
                                                            @endif
                                                ...
                                                {{-- Receiver --}}
                                                    <div class="form-group">
                                                        @if($transaction->transaction_type_id == Deposit
                                                                ...
                                                                || $transaction->transaction_type_id == Crypto_Exchange_From
                                                                || $transaction->transaction_type_id == Crypto_Exchange_To
                                                                || $transaction->transaction_type_id == Fiat_Exchange_From
                                                                || $transaction->transaction_type_id == Fiat_Exchange_To)
                                                        ...
                                                        <div class="col-sm-9">
                                                                <p class="form-control-static">
                                                                    ...
                                                                    @case(Crypto_Sent)
                                                                        {{ isset($transaction->end_user) ? $transaction->end_user->first_name . ' ' . $transaction->end_user->last_name : "-" }}
                                                                        @break
                                                                    @case(config('crypto_exchange.is_active') ? Crypto_Exchange_From : false)
                                                                        {{ isset($transaction->end_user) ? $transaction->end_user->first_name . ' ' . $transaction->end_user->last_name : "-" }}
                                                                        @break
                                                                    @case(config('crypto_exchange.is_active') ? Fiat_Exchange_From : false)
                                                                        {{ isset($transaction->end_user) ? $transaction->end_user->first_name . ' ' . $transaction->end_user->last_name : "-" }}
                                                                        @break
                                                                    @case(Transferred)
                                                                    @case(Received)
                                                                    ...
                                                                    @case(Crypto_Received)
                                                                        {{ isset($transaction->user) ? $transaction->user->first_name.' '.$transaction->user->last_name :"-" }}
                                                                        @break
                                                                    @case(config('crypto_exchange.is_active') ? Crypto_Exchange_To : false)
                                                                        {{ isset($transaction->end_user) ? $transaction->end_user->first_name.' '.$transaction->end_user->last_name :"-" }}
                                                                        @break
                                                                    @case(config('crypto_exchange.is_active') ? Fiat_Exchange_To : false)
                                                                        {{ isset($transaction->end_user) ? $transaction->end_user->first_name.' '.$transaction->end_user->last_name :"-" }}
                                                                        @break
                                                                @endswitch
                                                            </p>
                                                        ...
                                                        <div class="col-sm-9">
                                                            ...
                                                            @if (isset($transaction->refund_reference) && isset($transactionOfRefunded))
                                                                    ...
                                                                    <p class="form-control-static"><span class="label label-danger">Refund Reference: <i>
                                                                            <a id="transactionOfRefunded" href="{{  url(\Config::get('adminPrefix')."/transactions/edit/$transactionOfRefunded->id") }}">( {{ $transaction->refund_reference }} )</a>
                                                                        </i>
                                                                    </span>
                                                                    </p>
                                                            @elseif ($transaction->transaction_type_id == Crypto_Sent)
                                                            ...
                                                            @elseif (config('crypto_exchange.is_active') && $transaction->transaction_type_id == Crypto_Exchange_From)
                                                                    <p class="form-control-static"><span class="label label-danger" id="crypto-received-status" style="white-space: unset !important;">Crypto to Fiat Exchange Status Cannot Be Changed</span></p>
                                                            @elseif (config('crypto_exchange.is_active') && $transaction->transaction_type_id == Crypto_Exchange_To)
                                                                    <p class="form-control-static"><span class="label label-danger" id="crypto-received-status" style="white-space: unset !important;">Crypto to Fiat Exchange Status Cannot Be Changed</span></p>
                                                            @elseif (config('crypto_exchange.is_active') && $transaction->transaction_type_id == Fiat_Exchange_From)
                                                                    <p class="form-control-static"><span class="label label-danger" id="crypto-received-status" style="white-space: unset !important;">Fiat to Crypto Exchange Status Cannot Be Changed</span></p>
                                                            @elseif (config('crypto_exchange.is_active') && $transaction->transaction_type_id == Fiat_Exchange_To)
                                                                    <p class="form-control-static"><span class="label label-danger" id="crypto-received-status" style="white-space: unset !important;">Fiat to Crypto Exchange Status Cannot Be Changed</span></p>
                                                            @else
                                                ...
                                                <div class="row">
                                                    <div class="col-md-12">
                                                        <div class="row">
                                                            ...
                                                            <div class="col-md-2"><a id="cancel_anchor" class="btn btn-theme-danger pull-left" href="{{ url(\Config::get('adminPrefix').'/transactions') }}">Cancel</a></div>
                                                            @if (config('crypto_exchange.is_active') && $transaction->transaction_type_id != Crypto_Sent && $transaction->transaction_type_id != Crypto_Received && $transaction->transaction_type_id != Crypto_Exchange_From && $transaction->transaction_type_id != Crypto_Exchange_To && $transaction->transaction_type_id != Fiat_Exchange_From && $transaction->transaction_type_id != Fiat_Exchange_To)
                                                                @if (!isset($transaction->refund_reference))
                                                                    <div class="col-md-1">
                                                                        <button type="submit" class="btn btn-theme pull-right" id="request_payment">
                                                                            <i class="spinner fa fa-spinner fa-spin"></i> <span id="request_payment_text">Valider</span>
                                                                        </button>
                                                                    </div>
                                                                @endif
                                                            @endif

                                                            @if (!config('crypto_exchange.is_active') && $transaction->transaction_type_id != Crypto_Sent && $transaction->transaction_type_id != Crypto_Received)
                                                                @if (!isset($transaction->refund_reference))
                                                                    <div class="col-md-4">
                                                                        <button type="submit" class="btn btn-theme pull-right" id="request_payment">
                                                                            <i class="spinner fa fa-spinner fa-spin"></i> <span id="request_payment_text">Valider</span>
                                    ...
                                    <div class="mt-4 p-4 bg-secondary rounded shadow">
                                        @if ($transaction->subtotal)
                                            <div class="form-group">
                                                ...
                                                <div class="col-sm-6">
                                                    {{ ($transaction->currency->type != 'fiat' ) ? moneyFormat($transaction->currency->symbol, $transaction->subtotal) : moneyFormat($transaction->currency->symbol, formatNumber($transaction->subtotal)) }}
                                        ...
                                        <div class="form-group total-deposit-feesTotal-space">
                                            <label class="control-label col-sm-6" for="fee">{{ ($transaction->currency->type != 'fiat') ? 'Network Fees' : 'Fees' }}
                                                <span>
                                                    <small class="transactions-edit-fee">
                                                        @if (isset($transaction))
                                                            ...
                                                            @else
                                                                ({{(($transaction->transaction_type->name == "Payment_Sent") ? "0" : formatNumber($transaction->percentage))}}% + {{formatNumber($transaction->charge_percentage)}})
                                                            @endif
                                                        @else
                                                            ({{0}}%+{{0}})
                                                        @endif
                                            ...
                                            <div class="col-sm-6">
                                                <p class="form-control-static">
                                                    ...
                                                    @if (($transaction->currency->type != 'fiat' || ($transaction->transaction_type_id == Crypto_Sent)))
                                                        {{ $transaction->currency->symbol . ' ' .  number_format((isset($network_fee) ? $network_fee : '0.00000000'), 8, '.', ' ') }}
                                                    @else
                                        ...
                                        @if ($transaction->total)
                                            <div class="form-group total-deposit-space">
                                                ...
                                                <div class="col-sm-6">
                                                    <p class="form-control-static">
                                                        @if (($transaction->currency->type != 'fiat' || ($transaction->transaction_type_id == Crypto_Sent)))
                                                            {{ moneyFormat($transaction->currency->symbol, (number_format($transaction->subtotal + (isset($network_fee) ? $network_fee : 0.00000000), 8, '.', ''))) }}
                                                        @else
